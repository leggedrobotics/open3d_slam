...
class Mapper {
 public:
  ...
  Mapper(const TransformInterpolationBuffer& odomToRangeSensorBuffer, std::shared_ptr<SubmapCollection> submaps, const std::map<Time, Matrix6d>& covarianceBuffer);
  ~Mapper() = default;
