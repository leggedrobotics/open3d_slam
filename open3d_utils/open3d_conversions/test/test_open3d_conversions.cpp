...
TEST(ConversionFunctionsTgeometry, open3dToRos_uncolored_tgeometry) {
  ...
  for (int i = 0; i < 5; ++i) {
    o3d_vector_points.emplace_back(0.5 * i, i * i, 10.5 * i);
  }
  const open3d::core::Tensor o3d_tpc_points =
...
TEST(ConversionFunctionsTgeometry, open3dToRos_colored_tgeometry) {
  ...
  for (int i = 0; i < 5; ++i) {
    o3d_vector_points.emplace_back(0.5 * i, i * i, 10.5 * i);
    o3d_vector_colors.emplace_back(2 * i / 255.0, 5 * i / 255.0, 10 * i / 255.0);
  }
  const open3d::core::Tensor o3d_tpc_points =
      open3d::core::eigen_converter::EigenVector3dVectorToTensor(o3d_vector_points, dtype_f, device_type);
  const open3d::core::Tensor o3d_tpc_colors =
...
TEST(ConversionFunctionsTgeometry, rosToOpen3d_uncolored_tgeometry) {
  ...
  const open3d::core::Tensor o3d_tensor = o3d_tpc.GetPointPositions();
  EXPECT_EQ(ros_pc2_ptr->height * ros_pc2_ptr->width, o3d_tpc.GetPointPositions().GetShape()[0]);
  EXPECT_FALSE(o3d_tpc.HasPointColors());

  const std::vector<Eigen::Vector3d> point = open3d::core::eigen_converter::TensorToEigenVector3dVector(o3d_tensor);
  for (unsigned int i = 0; i < 5; ++i) {
...
TEST(ConversionFunctionsTgeometry, rosToOpen3d_colored_tgeometry) {
  ...
  open3d::t::geometry::PointCloud o3d_tpc;
  open3d_conversions::rosToOpen3d(ros_pc2_ptr, o3d_tpc);
  const open3d::core::Tensor o3d_points = o3d_tpc.GetPointPositions();
  const open3d::core::Tensor o3d_colors = o3d_tpc.GetPointColors();
  EXPECT_EQ(ros_pc2_ptr->height * ros_pc2_ptr->width, o3d_points.GetShape()[0]);
  EXPECT_TRUE(o3d_tpc.HasPointColors());

  const std::vector<Eigen::Vector3d> point = open3d::core::eigen_converter::TensorToEigenVector3dVector(o3d_points);
  const std::vector<Eigen::Vector3d> color = open3d::core::eigen_converter::TensorToEigenVector3dVector(o3d_colors);
  for (unsigned int i = 0; i < 5; ++i) {
    EXPECT_EQ(point[i](0), 0.5 * i);
    EXPECT_EQ(point[i](1), i * i);
    EXPECT_EQ(point[i](2), 10.5 * i);
    EXPECT_EQ(color[i](0), static_cast<float>(2 * i / 255.0));
    EXPECT_EQ(color[i](1), static_cast<float>(5 * i / 255.0));
    EXPECT_EQ(color[i](2), static_cast<float>(10 * i / 255.0));
  }
}
