...
void SlamWrapperRos::odomPublisherWorker() {
  ...
  while (ros::ok()) {
    ...
    if (!isAlreadyPublished && odometry_->hasProcessedMeasurements()) {
      const Transform T_odom_rangeSensor = odometry_->getOdomToRangeSensor(latestScanToScan);
      geometry_msgs::TransformStamped transformMsg = getTransformMsg(T_odom_rangeSensor, latestScanToScan);
    ...
    if (!isScanToMapAlreadyPublished && mapper_->hasProcessedMeasurements()) {
      const Transform T_map_rangeSensor = mapper_->getMapToRangeSensor(latestScanToMap);
      geometry_msgs::TransformStamped transformMsg = getTransformMsg(T_map_rangeSensor, latestScanToMap);
      nav_msgs::Odometry odomMsg = getOdomMsg(transformMsg);
...
void SlamWrapperRos::tfWorker() {
  ...
  while (ros::ok()) {
    ...
    if (!isAlreadyPublished && odometry_->hasProcessedMeasurements()) {
      ...
      o3d_slam::publishTfTransform(T_odom_rangeSensor.inverse().matrix(), timestamp, rangeSensorFrame, odomFrame, tfBroadcaster_.get());
