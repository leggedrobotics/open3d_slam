...
Parameters below affect scan to scan lidar odometry.
  ...
  scan_matching:
    ``cloud_registration_type`` - Which scan registration backend to use? Supported values are
    *GeneralizedIcp*, *PointToPlaneIcp* and *PointToPointIcp*.
    ...
    ``knn`` - Number of nearest neighbors used inside the ICP neighborhood search.

    ``max_distance_knn`` - Maximum search radius for the ICP neighbor lookup.
...
Parameters listed below affect scan to map refinement, map building, loop closure and pose graph optimization.
  ...
  scan_to_map_registration:
    ...
    ``scan_to_map_registration_type`` - same registration options as scan matching for odometry.

    icp:
      ``max_correspondence_dist`` - same as scan matching for odometry.

      ``knn`` - same as scan matching for odometry.

      ``max_distance_knn`` - same as scan matching for odometry.
